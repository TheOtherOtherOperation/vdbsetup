...
Empty lines and comments, those beginning with a hash ("#"), are ignored. Equal signs ("=") separate keys from values (major delimiter). If a parameter accepts multiple values, you may specify it as a comma-delimited (minor deliiter) list. Both the major and minor delimiter used may be overridden using the --major-delimiter and --minor-delimiter command-line flags.
...
- `distribution` (list, required if and only if "disttype=gaussian")
